...
export class OutputWindow {
    ...
    private static get MsgChannel() {
        ...
        if (!OutputWindow._msgChannel) {
            OutputWindow._msgChannel = vscode.window.createOutputChannel('babel es6');
        }
        return OutputWindow._msgChannel;
    }

    static Show(msgHeadline: string, MsgBody: string[], popUpToUI: boolean = true, addEndLine = true) {
